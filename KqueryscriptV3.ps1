...
$query = 'KubePodInventory| join (ContainerInventory | project ContainerID, ImageTag, ImageID) on $left.ContainerID == $right.ContainerID| where TimeGenerated > now(-5m)| where Namespace != "kube-system"| where ClusterName contains "jnwef-wv-aks"| summarize arg_max(TimeGenerated, *) by Name| order by Namespace asc, TimeGenerated desc'
