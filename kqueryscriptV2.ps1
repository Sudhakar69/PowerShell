...
$query = 'KubePodInventory| join (ContainerInventory | project ContainerID, ImageTag, ImageID) on $left.ContainerID == $right.ContainerID| where TimeGenerated > now(-5m)| where Namespace != "kube-system"| where ClusterName contains "dc-aks"| summarize arg_max(TimeGenerated, *) by Name| order by Namespace asc, TimeGenerated desc'
...
foreach($ServiceName in $ServiceNames) {
    ...
    for ($counter = 0; $counter -lt $maxPods; $counter++)
    {
        $report += "`n<tr>"
        if ($counter -eq 0) {
            $report += "`n`t<td rowspan=`"$maxPods`"><b>$ServiceName</b></td>"
        }
