param ([string]$info="Please provide paassword")

$jiraPATId=""
$path1 = "roject_reporting_common.ps1"
...
function Get-Employeesdata {
    $EmployeeData = New-Object -TypeName PSObject
    $EmployeeData| Add-Member -MemberType NoteProperty -Name Name -Value $null
    $EmployeeData| Add-Member -MemberType NoteProperty -Name EmailId -Value $null
    return $EmployeeData
...
$Availability = @()
for ($i = 2; $i -le $resourcesSheet.UsedRange.Rows.Count; $i++) {
...
$EmployeeInfo = @()
for ($j = 2; $j -le $EmployeeSheet.UsedRange.Rows.Count; $j++) {
    $name = $EmployeeSheet.Range("a$j").text
    $Emaild= $EmployeeSheet.Range("b$j").text
    # Write-Host $name "::" $Type "::" $date "::" $Hours
        $EmployeeData = Get-Employeesdata
        $EmployeeData.Name = $name
        $EmployeeData.EmailId = $Emaild
...
for ($j = 0; $j -le $Totalcount ; $j++) {
    $startAt = $j*100
    $EMADworkflowuri= "https://$($organization).atlassian.net/rest/api/2/search?jql=ORDER%20BY%20updated&maxResults=100&startAt=$($startAt)"
    $EMADworkflowResult = Invoke-RestMethod -Uri $EMADworkflowuri -Method Get -Headers $headers
    $EMAD = $EMADworkflowResult.issues.key |Where-Object{$_ -match "EMAD"}
    $keys = $EMAD|Select-Object -Unique |Sort-Object
    # $EMADkeys += $EMAD
# }
    # $keys = $EMADkeys|Select-Object -Unique |Sort-Object
    foreach($issueid in $keys) {
        ...
        if ($null -ne $worklogs) {    
            ...
            for ($i = 0; $i -lt $latestworklogs.Count; $i++) {
                ...
                $workitemstartedtime = $latestworklogs[$i].started
                [datetime]$WorkItemUpdate.updatedtime =$workitemstartedtime.ToUniversalTime()
                $workupdates += $WorkItemUpdate
            }
        }        
    }
}
$Assignees = $EmployeeInfo.name |Sort-Object
# $Assignees = ("")
[DateTime]$currentDateTime = Get-Date
# $WorkInfo = @()
foreach($Assignee in $Assignees){
    $AssigneeInfo = $EmployeeInfo | Where-Object {$_.name -eq $Assignee}
    $Emailid = $AssigneeInfo.EmailId
    [DateTime]$startDateUTC=([DateTime]$latestSprint.startDate).ToUniversalTime()
    [DateTime]$endDateUTC=([DateTime]$latestSprint.endDate).ToUniversalTime()
    [DateTime]$startDate=[System.TimeZoneInfo]::ConvertTimeBySystemTimeZoneId($startDateUTC, 'Central Europe Standard Time').toString()
    [DateTime]$endDate=[System.TimeZoneInfo]::ConvertTimeBySystemTimeZoneId($endDateUTC, 'Central Europe Standard Time').toString()
    ...
    $WorkitemSummaryHtml1 += "<br><b>Poland/Spain/France: </b> " + [System.TimeZoneInfo]::ConvertTimeBySystemTimeZoneId($today, 'Central Europe Standard Time').toString()
    ...
    $WorkitemSummaryHtml1 += "<br><b>Version: &nbsp; </b>V2.24" 
    $WorkitemSummaryHtml1 += "<br>"
    ...
    # $Assigneeemail = $workInfo.email | Select-Object -Unique
    ...
    if ($Totaltimespentinseconds -gt 0) {
    ...
    $issuesdetails = $workupdates |Where-Object{$_.AssignedTo -eq $Assignee -and $_.Timespent -gt 0 -and [datetime]$_.updatedtime -ge [datetime]$latestSprint.startDate}
    ...
    if ($issuesdetails.Count -eq 0 ) 
    {
        ...
        foreach($issue in $issuesCount){
            ...
            $Totaltimespent = New-TimeSpan -Seconds $Totaltimespentinseconds
            $Totaltimespentindays = $Totaltimespent.Days
            if ($Totaltimespentindays -gt 0) {
                $Totaltimespentinhours = "$($Totaltimespentindays)"+"Days "+"$($Totaltimespent.Hours)"+"H "+"$($Totaltimespent.Minutes)"+"Mins"
            }
            else {
                $Totaltimespentinhours = "$($Totaltimespent.Hours)"+"H "+"$($Totaltimespent.Minutes)"+"Mins"
            }
            $TotalEpicsTime += $Totaltimespentinseconds
            ...
            if ($TotalEpicsTime -gt 0) {
              $TotalEpicsTimespent  = New-TimeSpan -Seconds $TotalEpicsTime
              $totaldays = $TotalEpicsTimespent.Days
              if ($totaldays -gt 0) {
                $TotalEpicsTimespentinhours = "$($totaldays)"+"Days "+"$($TotalEpicsTimespent.Hours)"+"H "+"$($TotalEpicsTimespent.Minutes)"+"Mins"
              }
              else {
                $TotalEpicsTimespentinhours = "$($TotalEpicsTimespent.Hours)"+"H "+"$($TotalEpicsTimespent.Minutes)"+"Mins"
              }
              
                $EpicSummaryHtml += "<tr style='font-size:12px;' nowrap>
                    <td nowrap><b>&nbsp;  Total &nbsp;  </b></td>
                    <td align ='left' nowrap></td> 
                    <td align ='right' nowrap><b>&nbsp;  $($TotalEpicsTimespentinhours) &nbsp; </b></td>
                </tr>"
            }
        }
        ...
        foreach($issuesid in $issueids){
            ...
            $timespentonissue = New-TimeSpan -Seconds $timespentonissueinseconds
            $daysonissues = $timespentonissue.Days
            if ($daysonissues -gt 0) {
                $timespentonissueinhours = "$($daysonissues)"+"Days "+"$($timespentonissue.Hours)"+"H "+"$($timespentonissue.Minutes)"+"Mins"
            }
            else {
                $timespentonissueinhours = "$($timespentonissue.Hours)"+"H "+"$($timespentonissue.Minutes)"+"Mins"
            }
        ...
        if ($TotalTimeonissuesinseconds -gt 0) {
            $TotalTimeonissuesinhours = New-TimeSpan -Seconds $TotalTimeonissuesinseconds
            $days = $TotalTimeonissuesinhours.Days
            if ($days -gt 0) {
                $TotalTimeonissues = "$($days)"+"Days "+"$($TotalTimeonissuesinhours.Hours)"+"H "+"$($TotalTimeonissuesinhours.Minutes)"+"Mins"
            }
            else {
                $TotalTimeonissues = "$($TotalTimeonissuesinhours.Hours)"+"H "+"$($TotalTimeonissuesinhours.Minutes)"+"Mins"
            }
    ...
    # $email=("")
    $emailSubject = "Time Booking Summary for " +$latestSprint.Name+ "; " +$lefthours+" hour(s) to be booked in this sprint ( " +$Assignee+ "< " +$Emailid+ " > )"
    Start-Sleep -s 15
